 ...
 * @author rlozanov
 */
public enum TransportationMethod implements DatabaseResource {
    ...
    ;
    private TransportationMethod(String name, String description) {
        this.name = name;
    ...
    public static List<DbResource> getDbResources()
    {
        if(dbResources == null)
        {
            dbResources = new ArrayList<DbResource>(TransportationMethod.values().length);

            for(TransportationMethod unit : TransportationMethod.values())
            {
